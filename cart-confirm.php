...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
  ...
  if ($logged_in) {
    ...
    if ($existing) {
      // ★ 選択した数量分を加算
      $pdo->prepare('UPDATE carts SET quantity = quantity + ? WHERE customer_id = ? AND product_id = ?')
          ->execute([$quantity, $customer_id, $product_id]);
    } else {
      // ★ はじめから選択数量で INSERT
      $pdo->prepare('INSERT INTO carts (customer_id, product_id, quantity, added_at) VALUES (?, ?, ?, NOW())')
          ->execute([$customer_id, $product_id, $quantity]);
    }
